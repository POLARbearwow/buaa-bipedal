...
def joint_vel_l2_stand(
    env: ManagerBasedRLEnv,
    command_name: str,
    command_threshold: float = 0.05,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize joint velocity magnitude for standing environments (command <= threshold)."""
    asset: Articulation = env.scene[asset_cfg.name]
    ...
    return reward * (command <= command_threshold)


def joint_vel_l2_moving(
    env: ManagerBasedRLEnv,
    command_name: str,
    command_threshold: float = 0.05,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize joint velocity magnitude for moving environments (command > threshold)."""
    asset: Articulation = env.scene[asset_cfg.name]
    command = torch.norm(env.command_manager.get_command(command_name), dim=1)
    reward = torch.sum(torch.square(asset.data.joint_vel[:, asset_cfg.joint_ids]), dim=1)
    return reward * (command > command_threshold)
...
def feet_air_time_positive_biped(
    env, command_name: str, threshold: float, sensor_cfg: SceneEntityCfg, command_threshold: float = 0.1
) -> torch.Tensor:
...
def fly(env: ManagerBasedRLEnv, threshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    """Penalize a walking robot when both feet are off the ground."""
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    net_contact_forces = contact_sensor.data.net_forces_w_history
    contact_force = torch.linalg.norm(net_contact_forces[:, :, sensor_cfg.body_ids], dim=-1)
    is_contact = torch.max(contact_force, dim=1).values > threshold
...
def phase_conditioned_contact(
    ...
    """Reward smooth, phase-conditioned left/right foot contact."""
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    ...
    phase = torch.remainder(
        phase[:, None] + torch.tensor([phase_offset_l, phase_offset_r], device=env.device), 1.0
    )

    # Match TienKung's gait clock: swing starts at phase zero, with a linear
    # transition around both swing boundaries for a usable reward signal.
    width = max(float(transition_width), 1e-6)
    swing = torch.zeros_like(phase)
    swing = torch.where(
        phase < width,
        0.5 + phase / (2.0 * width),
        swing,
    ...
    swing = torch.where(
        (phase >= width) & (phase <= air_ratio - width),
        torch.ones_like(swing),
        swing,
    )
    swing = torch.where(
        (phase > air_ratio - width) & (phase < air_ratio + width),
        (air_ratio + width - phase) / (2.0 * width),
        swing,
    )
    swing = torch.where(
        phase > 1.0 - width,
        (phase - 1.0 + width) / (2.0 * width),
        swing,
    )
    swing = torch.clamp(swing, 0.0, 1.0)
    desired_contact = 1.0 - swing
    contact_float = contact.float()
    reward = torch.mean(
        contact_float * desired_contact + (1.0 - contact_float) * (1.0 - desired_contact), dim=1
    )
...
def feet_y_distance(
    ...
    """Penalize deviation from the target lateral distance between both feet.

    The penalty is active only when the commanded lateral velocity is small,
    so turning or lateral walking is not constrained by the nominal stance.
    ``asset_cfg.body_ids`` must contain the left foot followed by the right foot.
    """
    asset: RigidObject = env.scene[asset_cfg.name]
    if len(asset_cfg.body_ids) != 2:
        raise ValueError("feet_y_distance requires exactly two foot bodies")

    foot_offset = asset.data.body_link_pos_w[:, asset_cfg.body_ids, :] - asset.data.root_link_pos_w[:, None, :]
    foot_offset_b = torch.empty_like(foot_offset)
    root_quat_conjugate = math_utils.quat_conjugate(asset.data.root_link_quat_w)
    for foot_idx in range(2):
        foot_offset_b[:, foot_idx, :] = math_utils.quat_apply(
            root_quat_conjugate, foot_offset[:, foot_idx, :]
        )
    y_distance = torch.abs(foot_offset_b[:, 0, 1] - foot_offset_b[:, 1, 1])
    distance_error = torch.abs(y_distance - target_distance)
    # low_lateral_command = torch.abs(env.command_manager.get_command(command_name)[:, 1]) < 0.1
...
def base_height_tracking(
    ...
    """Reward keeping the base at a target world height.

    The coefficient matches RoboTamer's ``exp(-70 * (z - 0.45)^2)`` term.
    """
    asset: RigidObject = env.scene[asset_cfg.name]
    return torch.exp(-70.0 * torch.square(asset.data.root_pos_w[:, 2] - target_height))


def base_orientation_tracking(
    env: ManagerBasedEnv,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    coefficient: float = 5.0,
) -> torch.Tensor:
    """Reward an upright base, using the projected gravity roll/pitch error."""
    asset: RigidObject = env.scene[asset_cfg.name]
    tilt_error = torch.sum(torch.square(asset.data.projected_gravity_b[:, :2]), dim=1)
    return torch.exp(-coefficient * tilt_error)
...
def joint_pos_tracking_l2(
    ...
    """Penalize the position-target tracking error of the actuated joints.

    This is distinct from :func:`joint_pos_penalty`, which measures deviation
    from the articulation default pose.
    """
    asset: Articulation = env.scene[asset_cfg.name]
    error = asset.data.joint_pos_target[:, asset_cfg.joint_ids] - asset.data.joint_pos[:, asset_cfg.joint_ids]
