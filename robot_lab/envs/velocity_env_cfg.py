...
# Sky HDRI that ships with Isaac Sim.  It is hosted on NVIDIA's asset CDN by default, which is
# slow or unreachable from some networks; Isaac Sim then blocks scene creation for minutes while
# fetching it.  Only reference the texture when it resolves to a local file (for example through
# the mirror set up by ``scripts/local_assets.py``); otherwise fall back to a plain dome light.
_SKY_HDRI_FILE = f"{ISAAC_NUCLEUS_DIR}/Materials/Textures/Skies/PolyHaven/kloofendal_43d_clear_puresky_4k.hdr"
SKY_HDRI_FILE = _SKY_HDRI_FILE if os.path.exists(_SKY_HDRI_FILE) else None
...
class RewardsCfg:
    ...
    base_height_tracking = RewTerm(
        ...
        weight=0.0,
        params={"asset_cfg": SceneEntityCfg("robot", body_names=""), "target_height": 0.29},
    )
    base_orientation_tracking = RewTerm(
        func=mdp.base_orientation_tracking,
        weight=0.0,
        params={"asset_cfg": SceneEntityCfg("robot", body_names=""), "coefficient": 5.0},
    )
    ...
    joint_vel_l2_stand = RewTerm(
        func=mdp.joint_vel_l2_stand,
        weight=0.0,
        params={
            "command_name": "base_velocity",
            "command_threshold": 0.05,
    ...
    joint_vel_l2_moving = RewTerm(
        func=mdp.joint_vel_l2_moving,
        weight=0.0,
        params={
            "command_name": "base_velocity",
            "command_threshold": 0.05,
