...
class AMPLoader:
    def __init__(
        ...
        self.END_EFFECTOR_POS_SIZE = end_effector_pos_size
        ...
        if not motion_files:
            raise ValueError("AMPLoader received no motion files")
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file) as f:
                ...
                if motion_data.ndim != 2 or motion_data.shape[1] != self.END_POS_END_IDX:
                    raise ValueError(
                        f"{motion_file} must contain [num_frames, {self.END_POS_END_IDX}] frames, "
                        f"got {motion_data.shape}"
                    )
