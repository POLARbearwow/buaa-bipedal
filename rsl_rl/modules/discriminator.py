...
class Discriminator(nn.Module):
    ...
    def predict_amp_reward(self, state, next_state, task_reward, normalizer=None, reward_gate=None):
        """
        ...
        Args:
            ...
            reward_gate (torch.Tensor | None): Optional per-environment multiplier applied to the AMP
                reward before it is interpolated with the task reward. A value of ``0`` mutes the AMP
                reward (e.g. for environments that are commanded to stand still) while keeping the
                task reward.
        ...
        with torch.no_grad():
            ...
            if reward_gate is not None:
                amp_reward = amp_reward * reward_gate.reshape(amp_reward.shape).to(amp_reward.device)
            self.last_amp_reward = amp_reward.mean().detach()
            self.last_task_reward = task_reward.mean().detach()
            reward = amp_reward
